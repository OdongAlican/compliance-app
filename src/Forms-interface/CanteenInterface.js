...
/* ── SetupFormModal ────────────────────────────────────────────────────── */
function SetupFormModal({ isOpen, onClose, setup, users, catalogItems }) {
  ...
  useEffect(() => {
    if (isOpen) {
      if (setup) {
        ...
        setForm({ name: "", location: "", date: "", time: "", note: "", safety_officer_id: "", supervisor_id: "", inspection_id: "" });
      }
      setErrors({});
  ...
  const soList =
    users.filter((u) =>
      (u.role?.name || u.role || "").toLowerCase().includes("safety")
    ).length
      ? users.filter((u) =>
        (u.role?.name || u.role || "").toLowerCase().includes("safety")
      )
  ...
  const supList =
    users.filter((u) =>
      (u.role?.name || u.role || "").toLowerCase().includes("supervisor")
    ).length
      ? users.filter((u) =>
        (u.role?.name || u.role || "").toLowerCase().includes("supervisor")
      )
      ...
      <form onSubmit={handleSubmit} className="p-6 grid grid-cols-2 gap-4">
        ...
        <div className="col-span-2 sm:col-span-1">
          <Field label="Safety Officer" required error={errors.safety_officer_id}>
            <select
              value={form.safety_officer_id}
              onChange={(e) => set("safety_officer_id", e.target.value)}
              className="ui-input text-sm"
            >
        ...
        <div
          className="col-span-2 flex justify-end gap-3 pt-2"
