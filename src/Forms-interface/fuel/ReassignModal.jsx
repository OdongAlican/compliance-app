...
export default function ReassignModal({ isOpen, mode, setupId, onClose }) {
  ...
  async function handleSave() {
    ...
    const action = isSupervisor
      ? dispatch(reassignFuelSupervisor({ id: setupId, supervisorId: Number(selectedUser.id) }))
      : dispatch(reassignFuelSafetyOfficer({ id: setupId, safetyOfficerId: Number(selectedUser.id) }));
    const result = await action;
    if (
      reassignFuelSupervisor.fulfilled.match(result) ||
      reassignFuelSafetyOfficer.fulfilled.match(result)
    ) {
