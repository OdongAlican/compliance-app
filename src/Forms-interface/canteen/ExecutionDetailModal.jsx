...
import {
  CanteenPerformService,
  CanteenChecklistItemService,
  CanteenIssueService,
  CanteenAttachmentService,
  CanteenRepairAttachmentService,
...
function AttachmentList({ performId, issueId, kind = "issue" }) {
  /* kind: "issue" | "repair" */
  const svc = kind === "repair" ? CanteenRepairAttachmentService : CanteenAttachmentService;
...
function IssueCard({ issue, performId, onDeleted, onUpdated }) {
  ...
  async function saveCA() {
    setSaving(true);
    try {
      const res = await CanteenIssueService.updateCorrectiveAction(performId, issue.id, caText);
  ...
  async function savePriority() {
    setSaving(true);
    try {
      const res = await CanteenIssueService.updatePriorityDueDate(performId, issue.id, {
  ...
  async function saveContractor() {
    ...
    setSaving(true);
    try {
      const res = await CanteenIssueService.assignContractor(performId, issue.id, contractor.id);
  ...
  async function saveRepairCompletion() {
    setSaving(true);
    try {
      await CanteenIssueService.repairCompletion(
        performId,
        issue.id,
  ...
  async function saveRepairStatus() {
    ...
    setSaving(true);
    try {
      const res = await CanteenIssueService.setRepairStatus(
  ...
  async function handleDelete() {
    setDeleting(true);
    try {
      await CanteenIssueService.remove(performId, issue.id);
...
function AddIssueForm({ performId, onAdded, onCancel }) {
  ...
  async function handleSubmit() {
    ...
    setSaving(true);
    try {
      const res = await CanteenIssueService.create(performId, form, files);
...
/* ── ChecklistSection ─────────────────────────────────────────────────── */
function ChecklistSection({ performId, performData }) {
  ...
  const load = useCallback(async () => {
    ...
    setLoading(true);
    try {
      const res = await CanteenPerformService.get(performId, { include: "checklists" });
  ...
  async function saveChecklist(pc) {
    ...
    try {
      ...
      await CanteenChecklistItemService.upsert(performId, pc.id, items);
  ...
  return (
    <div className="flex flex-col gap-3">
      {checklists.map((pc) => {
        ...
        /* Summary counts */
        const sat = tmplItems.filter((i) => (pcResults[i.id]?.status ?? "satisfactory") === "satisfactory").length;
        const attn = tmplItems.filter((i) => pcResults[i.id]?.status === "needs_attention").length;
        const na = tmplItems.filter((i) => pcResults[i.id]?.status === "not_applicable").length;
                ...
                {tmplItems.length === 0 ? (
                  ...
                  <>
                    <div className="flex flex-col divide-y" style={{ borderColor: "var(--border)" }}>
                      {tmplItems.map((item) => {
                        const val = pcResults[item.id] ?? { status: "satisfactory", comment: "" };
                        return (
                          <div key={item.id} className="px-4 py-3">
                            <div className="flex items-start justify-between gap-3">
                              ...
                              <div className="flex gap-1 flex-shrink-0">
                                {STATUS_ITEM_OPTIONS.map((opt) => {
                                  ...
                                  const col = STATUS_ITEM_COLOR[opt.value];
                                  return (
                                    <button
                                      key={opt.value}
                                      onClick={() => setItemField(pc.id, item.id, "status", opt.value)}
                                      className="text-[10px] font-bold px-2 py-1 rounded-full"
                                      style={{
                                        background: active ? col.bg : "transparent",
                                        color: active ? col.text : "var(--text-muted)",
                                        border: `1px solid ${active ? col.text : "var(--border)"}`,
                                        transition: "all 0.1s",
                                      }}
                                    >
                                      {opt.label}
                            ...
                            <input
                              type="text"
                              value={val.comment}
                              onChange={(e) => setItemField(pc.id, item.id, "comment", e.target.value)}
                              placeholder="Comment (optional)…"
                              className="ui-input text-xs mt-2 w-full"
                              style={{ fontSize: "12px" }}
                            />
                          </div>
                        );
...
function IssueSection({ performId }) {
  ...
  const load = useCallback(async () => {
    ...
    setLoading(true);
    try {
      const res = await CanteenIssueService.list(performId, { per_page: 50 });
        ...
        >
          <div>
            ...
            <p className="text-xs" style={{ color: "var(--text-muted)" }}>
              {setup?.name} · {perform.date} at {perform.time}
            </p>
            {perform.note && (
              <p className="text-xs mt-0.5" style={{ color: "var(--text-muted)" }}>
                {perform.note}
