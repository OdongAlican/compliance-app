/* ── Science Lab Inspection — ReassignModal ──────────────────────────── */
import { useState, useEffect } from "react";
import { UserIcon } from "@heroicons/react/24/outline";
import { useAppDispatch, useAppSelector } from "../../store/hooks";
import {
  reassignScienceLabSupervisor,
  reassignScienceLabSafetyOfficer,
...
export default function ReassignModal({ isOpen, onClose, mode, setupId }) {
  ...
  async function handleSave() {
    ...
    const action = isSupervisor
      ? dispatch(reassignScienceLabSupervisor({ id: setupId, supervisorId: Number(selectedUser.id) }))
      : dispatch(reassignScienceLabSafetyOfficer({ id: setupId, safetyOfficerId: Number(selectedUser.id) }));
    const result = await action;
    if (
      reassignScienceLabSupervisor.fulfilled.match(result) ||
      reassignScienceLabSafetyOfficer.fulfilled.match(result)
    ) {
