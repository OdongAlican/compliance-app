/* ── Science Lab Inspection — SetupFormModal ─────────────────────────── */
import { useState, useEffect } from "react";
import { CheckBadgeIcon } from "@heroicons/react/24/outline";
import { useAppDispatch, useAppSelector } from "../../store/hooks";
import {
  createScienceLabSetup,
  updateScienceLabSetup,
  clearScienceLabActionError,
  selectScienceLabActionLoading,
  selectScienceLabActionError,
...
export default function SetupFormModal({ isOpen, onClose, setup }) {
  ...
  const actionLoading = useAppSelector(selectScienceLabActionLoading);
  const actionError = useAppSelector(selectScienceLabActionError);
  ...
  useEffect(() => {
    if (isOpen) {
      ...
      if (setup) {
        setForm({
          laboratory_name: setup.laboratory_name ?? "",
          date: setup.date ?? "",
          time: setup.time ?? "",
          note: setup.note ?? "",
        });
        ...
        setForm({ laboratory_name: "", date: "", time: "", note: "" });
  ...
  function validateStep(s) {
    const e = {};
    if (s === 1) {
      if (!form.laboratory_name.trim()) e.laboratory_name = "Laboratory name is required.";
  ...
  async function handleSubmit() {
    const payload = {
      laboratory_name: form.laboratory_name.trim(),
      date: form.date,
      time: form.time,
    ...
    const action = isEdit
      ? dispatch(updateScienceLabSetup({ id: setup.id, data: payload }))
      : dispatch(createScienceLabSetup(payload));
    const result = await action;
    if (
      createScienceLabSetup.fulfilled.match(result) ||
      updateScienceLabSetup.fulfilled.match(result)
    ) {
      ...
      return (
        <div className="p-6 grid grid-cols-2 gap-4">
          <div className="col-span-2">
            <Field label="Laboratory Name" required error={errors.laboratory_name}>
              <input
      ...
      return (
        <div className="p-6 flex flex-col gap-4">
          ...
          <div className="grid grid-cols-2 gap-3">
            <div className="col-span-2">
              <ReviewRow label="Laboratory Name" value={form.laboratory_name} />
            </div>
  ...
  return (
    <ModalShell
      isOpen={isOpen}
      onClose={onClose}
      title={isEdit ? "Edit Science Lab Inspection" : "New Science Lab Inspection"}
      width="max-w-lg"
    >
