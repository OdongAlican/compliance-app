...
import { ScienceLabPerformService } from "../../services/scienceLab.service";
...
export default function DetailDrawer({ isOpen, onClose, setup }) {
  ...
  const load = useCallback(async () => {
    ...
    setError("");
    try {
      const res = await ScienceLabPerformService.list(setup.id, { per_page: 20 });
  ...
  async function handleSignOff(pid) {
    setSigning(pid);
    try {
      await ScienceLabPerformService.signOff(pid);
          ...
          <div className="flex items-start gap-3">
            ...
            <div>
              ...
              <p className="text-xs mt-0.5" style={{ color: "var(--text-muted)" }}>
                Science Lab Inspection · #{setup.id}
              </p>
              <span
