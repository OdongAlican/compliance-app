...
import {
  fetchScienceLabSetups,
  deleteScienceLabSetup,
  setScienceLabPage,
  setScienceLabLabName,
  setScienceLabDateFrom,
  setScienceLabDateTo,
  clearScienceLabFilters,
  clearScienceLabError,
  clearScienceLabActionError,
  selectScienceLabSetups,
  selectScienceLabMeta,
  selectScienceLabLoading,
  selectScienceLabError,
  selectScienceLabFilters,
} from "../store/slices/scienceLabSlice";
import useAuth from "../hooks/useAuth";
import { STATUS_STYLE } from "./sciencelab/constants";
import { Pagination, TableSkeleton } from "./sciencelab/shared";
import ActionMenu from "./sciencelab/ActionMenu";
import SetupFormModal from "./sciencelab/SetupFormModal";
import ReassignModal from "./sciencelab/ReassignModal";
...
export default function ScienceLabInterface() {
  ...
  const setups = useAppSelector(selectScienceLabSetups);
  const meta = useAppSelector(selectScienceLabMeta);
  const loading = useAppSelector(selectScienceLabLoading);
  const error = useAppSelector(selectScienceLabError);
  const filters = useAppSelector(selectScienceLabFilters);
  ...
  useEffect(() => {
    dispatch(fetchScienceLabSetups());
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [dispatch, filters.page, filters.laboratory_name, filters.date_from, filters.date_to]);

  /* Surface API errors via toast */
  useEffect(() => {
    if (error) {
      toast.error(error);
      dispatch(clearScienceLabError());
    }
  }, [error, dispatch]);
  ...
  function handleSearch(val) {
    ...
    searchTimer.current = setTimeout(() => dispatch(setScienceLabLabName(val)), 400);
  }

  async function handleDelete() {
    if (!deleteTarget) return;
    setDeleting(true);
    const result = await dispatch(deleteScienceLabSetup(deleteTarget.id));
    setDeleting(false);
    setDeleteTarget(null);
    if (deleteScienceLabSetup.fulfilled.match(result)) toast.success("Inspection deleted.");
    else toast.error(result.payload || "Delete failed.");
  }

  function handleExport() {
    const rows = [
      ["ID", "Laboratory Name", "Date", "Time", "Safety Officer", "Supervisor"],
      ...setups.map((s) => [
        s.id,
        s.laboratory_name,
        s.date,
    ...
    const a = Object.assign(document.createElement("a"), {
      href: url,
      download: "science-lab-inspections.csv",
    });
  ...
  const hasFilters = !!(filters.laboratory_name || filters.date_from || filters.date_to);
  ...
  return (
    <div className="ui-page" style={{ color: "var(--text)" }}>
          ...
          <div>
            ...
            <p className="text-xs mt-0.5" style={{ color: "var(--text-muted)" }}>
              {meta
                ...
                : "Manage science lab inspection setups."}
            </p>
          </div>
        </div>
        <div className="flex items-center gap-2">
          <button
            onClick={() => dispatch(fetchScienceLabSetups())}
            disabled={loading}
      ...
      <div className="ui-card mb-4 p-3">
        <div className="flex flex-col sm:flex-row gap-3 items-stretch sm:items-center flex-wrap">
          ...
          >
            ...
            {searchInput && (
              <button
                onClick={() => { setSearchInput(""); dispatch(setScienceLabLabName("")); }}
                style={{ color: "var(--text-muted)" }}
              >
          ...
          >
            ...
            <input
              ...
              onChange={(e) => dispatch(setScienceLabDateFrom(e.target.value))}
              title="From date"
              style={{
                background: "transparent",
                outline: "none",
            ...
            <input
              ...
              onChange={(e) => dispatch(setScienceLabDateTo(e.target.value))}
              title="To date"
              style={{
                background: "transparent",
                outline: "none",
          ...
          {hasFilters && (
            <button
              onClick={() => { setSearchInput(""); dispatch(clearScienceLabFilters()); }}
      ...
      <div className="ui-card">
                ...
                <tr>
                  ...
                  >
                    {hasFilters
                      ...
                      : "No science lab inspections yet."}
                  </td>
                </tr>
              </tbody>
            ) : (
        ...
        <Pagination meta={meta} onPage={(p) => dispatch(setScienceLabPage(p))} />
      ...
      <SetupFormModal
        ...
        onClose={() => {
          ...
          dispatch(clearScienceLabActionError());
        }}
