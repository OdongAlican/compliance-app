/* ── Vehicle Inspection — ReassignModal ──────────────────────────────── */
import { useState, useEffect } from "react";
import { UserIcon } from "@heroicons/react/24/outline";
import { useAppDispatch, useAppSelector } from "../../store/hooks";
import {
  reassignVehicleSupervisor,
  reassignVehicleSafetyOfficer,
...
export default function ReassignModal({ isOpen, onClose, mode, setupId }) {
  ...
  async function handleSave() {
    ...
    const action = isSupervisor
      ? dispatch(reassignVehicleSupervisor({ id: setupId, supervisorId: Number(selectedUser.id) }))
      : dispatch(reassignVehicleSafetyOfficer({ id: setupId, safetyOfficerId: Number(selectedUser.id) }));
    const result = await action;
    if (
      reassignVehicleSupervisor.fulfilled.match(result) ||
      reassignVehicleSafetyOfficer.fulfilled.match(result)
    ) {
        ...
        >
          ...
          <div>
            ...
            <p className="text-xs mt-0.5" style={{ color: "var(--text-muted)" }}>
              Search by name to find and select a new{" "}
              {isSupervisor ? "supervisor" : "safety officer"} for this inspection.
            </p>
          </div>
        </div>
