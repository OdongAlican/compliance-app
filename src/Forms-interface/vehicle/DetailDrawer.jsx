/* ── Vehicle Inspection — DetailDrawer ───────────────────────────────── */
...
export default function DetailDrawer({ isOpen, onClose, setup }) {
  ...
  const load = useCallback(async () => {
    ...
    setError("");
    try {
      const res = await VehiclePerformService.list(setup.id, { per_page: 20 });
  ...
  async function handleSignOff(pid) {
    setSigning(pid);
    try {
      await VehiclePerformService.signOff(pid);
      ...
      >
        ...
        <div className="px-6 py-5" style={{ borderBottom: "1px solid var(--border)" }}>
          ...
          <div className="grid grid-cols-2 gap-3 mb-4">
            <ReviewRow label="Vehicle ID" value={setup.vehicle_id} />
            <ReviewRow label="Model" value={setup.model} />
                    ...
                    >
                      ...
                      {!signed && (
                        <button
                          ...
                          className="flex items-center gap-1.5 text-xs font-semibold px-3 py-1.5 rounded-lg flex-shrink-0 disabled:opacity-60"
                          style={{
                            background: "color-mix(in srgb,#3fb950 15%,transparent)",
                            color: "#3fb950",
