/* ── Hand & Power Tools Inspection — StartInspectionModal ─────────── */
...
export default function StartInspectionModal({ isOpen, onClose, setup }) {
  ...
  /* Load checklist templates using setup's inspection id (auto-set by backend) */
  useEffect(() => {
  ...
  async function handleSubmit() {
    ...
    try {
      ...
      await ToolPerformService.create(setup.id, {
        perform: {
          date: form.date,
          time: form.time,
  ...
  return (
    <ModalShell
      isOpen={isOpen}
      onClose={onClose}
      title={"Start Inspection — " + (setup?.tool_name ?? "")}
      width="max-w-3xl"
    >
