...
function IssueCard({ issue, performId, onDeleted, onUpdated }) {
    ...
    async function saveCA() {
        setSaving(true);
        try {
            await ToolIssueService.updateCorrectiveAction(performId, issue.id, caText);
    ...
    async function savePriority() {
        setSaving(true);
        try {
            await ToolIssueService.updatePriorityDueDate(performId, issue.id, {
    ...
    async function saveContractor() {
        ...
        setSaving(true);
        try {
            await ToolIssueService.assignContractor(performId, issue.id, contractor.id);
    ...
    async function saveRepairCompletion() {
        setSaving(true);
        try {
            await ToolIssueService.repairCompletion(
                performId,
                issue.id,
    ...
    async function saveRepairStatus() {
        ...
        setSaving(true);
        try {
            await ToolIssueService.setRepairStatus(performId, issue.id, repairStatus, rejectReason);
    ...
    async function handleDelete() {
        setDeleting(true);
        try {
            await ToolIssueService.remove(performId, issue.id);
...
function AddIssueForm({ performId, onAdded, onCancel }) {
    ...
    async function handleSubmit() {
        ...
        setSaving(true);
        try {
            const res = await ToolIssueService.create(performId, form, files);
...
function ChecklistSection({ performId }) {
    ...
    const load = useCallback(async () => {
        ...
        setLoading(true);
        try {
            const res = await ToolPerformService.get(performId, { include: "checklists" });
            const data = res.data ?? res;
            const pcs = data.performed_hand_power_tools_checklists ?? [];
            setChecklists(pcs);
            const init = {};
            pcs.forEach((pc) => {
                init[pc.id] = {};
                (pc.performed_hand_power_tools_checklist_items ?? []).forEach((pci) => {
    ...
    async function saveChecklist(pc) {
        ...
        try {
            ...
            await ToolChecklistItemService.upsert(performId, pc.id, items);
    ...
    const _allItems = checklists.flatMap((pc) =>
        (pc.performed_hand_power_tools_checklist_items ?? []).map((i) => ({
            pcId: pc.id,
    ...
    return (
        <div className="flex flex-col gap-3">
            ...
            {checklists.map((pc) => {
                ...
                const performedItems = pc.performed_hand_power_tools_checklist_items ?? [];
...
function IssueSection({ performId }) {
    ...
    const load = useCallback(async () => {
        ...
        setLoading(true);
        try {
            const res = await ToolIssueService.list(performId, { per_page: 50 });
...
export default function ExecutionDetailModal({ isOpen, onClose, perform, setup }) {
    ...
    const toolName = perform.hand_power_tools_inspection?.tool_name ?? setup?.tool_name ?? "";
    const toolId = perform.hand_power_tools_inspection?.tool_id ?? setup?.tool_id ?? "";
    const location = perform.hand_power_tools_inspection?.location ?? setup?.location ?? "";
