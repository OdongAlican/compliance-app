/* ── Hand & Power Tools Inspection — DetailDrawer ────────────────────── */
...
import { ToolPerformService } from "../../services/tool.service";
...
export default function DetailDrawer({ isOpen, onClose, setup }) {
  ...
  const load = useCallback(async () => {
    ...
    setError("");
    try {
      const res = await ToolPerformService.list(setup.id, { per_page: 20 });
  ...
  async function handleSignOff(pid) {
    setSigning(pid);
    try {
      await ToolPerformService.signOff(pid);
      ...
      >
        ...
        >
          <div className="flex items-start gap-3">
            ...
            >
              <WrenchScrewdriverIcon className="w-5 h-5" style={{ color: "var(--accent)" }} />
            </div>
            <div>
              ...
              <p className="text-xs mt-0.5" style={{ color: "var(--text-muted)" }}>
                Hand &amp; Power Tools Inspection · #{setup.id}
              </p>
              <span
        ...
        <div className="px-6 py-5" style={{ borderBottom: "1px solid var(--border)" }}>
          ...
          <div className="grid grid-cols-2 gap-3 mb-4">
            <ReviewRow label="Tool Name" value={setup.tool_name} />
            <ReviewRow label="Tool ID" value={setup.tool_id} />
