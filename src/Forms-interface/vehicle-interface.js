...
export default function VehicleInterface() {
  ...
  const filteredData = data.filter((entry) => {
    ...
    const matchesSearch =
      entry.vehiclename.toLowerCase().includes(searchTerm.toLowerCase()) ||
  ...
  return (
    <div className="p-6 max-w-7xl mx-auto bg-gray-50 min-h-screen">
      <h1 className="text-2xl font-bold text-gray-800 mb-6">Vehicle Inspection</h1>
          ...
          <tbody>
            {filteredData.map((entry) => (
              <tr key={entry.id} className="hover:bg-gray-50">
                ...
                <td className="border px-4 py-2 text-sm">{entry.vehiclename}</td>
