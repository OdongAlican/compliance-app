...
export default function DetailDrawer({ isOpen, onClose, setup }) {
  ...
  const load = useCallback(async () => {
    ...
    setError("");
    try {
      const res = await PpePerformService.list(setup.id, { per_page: 20 });
  ...
  async function handleSignOff(pid) {
    setSigning(pid);
    try {
      await PpePerformService.signOff(pid);
  ...
  const ppeUserObj = setup.ppe_user;
  const ppeUserName = ppeUserObj ? ppeUserObj.firstname + " " + ppeUserObj.lastname : null;
  const ppeUserInitials = ppeUserObj
    ? (ppeUserObj.firstname?.[0] ?? "") + (ppeUserObj.lastname?.[0] ?? "")
    : "?";

  const so = setup.safety_officer;
  const soName = so ? so.firstname + " " + so.lastname : null;
  const soInitials = so ? (so.firstname?.[0] ?? "") + (so.lastname?.[0] ?? "") : "?";

  const sup = setup.supervisor;
  const supName = sup ? sup.firstname + " " + sup.lastname : null;
  const supInitials = sup ? (sup.firstname?.[0] ?? "") + (sup.lastname?.[0] ?? "") : "?";
        ...
        <div className="px-6 py-5" style={{ borderBottom: "1px solid var(--border)" }}>
          ...
          <div className="grid grid-cols-2 gap-3 mb-4">
            <div className="col-span-2">
              <ReviewRow label="Department" value={setup.department} />
            </div>
            {ppeUserName && (
              <div className="col-span-2">
                <ReviewRow label="PPE User" value={ppeUserName} />
              </div>
            )}
          ...
          <div className="grid grid-cols-3 gap-3">
            {/* PPE User */}
            <div
              className="p-3 rounded-xl flex items-center gap-3 col-span-1"
