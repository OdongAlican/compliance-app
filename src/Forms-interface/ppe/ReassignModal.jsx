/* ── PPE Inspection — ReassignModal ───────────────────────────────────── */
import { useState, useEffect } from "react";
import { UserIcon } from "@heroicons/react/24/outline";
import { useAppDispatch, useAppSelector } from "../../store/hooks";
import {
  reassignPpeSupervisor,
  reassignPpeSafetyOfficer,
...
export default function ReassignModal({ isOpen, mode, setupId, onClose }) {
  ...
  const actionLoading = useAppSelector(selectPpeActionLoading);
  ...
  async function handleSave() {
    ...
    const action = isSupervisor
      ? dispatch(reassignPpeSupervisor({ id: setupId, supervisorId: Number(selectedUser.id) }))
      : dispatch(reassignPpeSafetyOfficer({ id: setupId, safetyOfficerId: Number(selectedUser.id) }));
    const result = await action;
    if (
      reassignPpeSupervisor.fulfilled.match(result) ||
      reassignPpeSafetyOfficer.fulfilled.match(result)
    ) {
