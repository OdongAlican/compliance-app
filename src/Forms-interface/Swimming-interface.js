...
import {
  ...
  selectSwimmingPoolSetups,
  selectSwimmingPoolSetupsMeta,
  selectSwimmingPoolLoading,
  selectSwimmingPoolError,
  selectSwimmingPoolFilters,
} from "../store/slices/swimmingPoolSlice";
import useAuth from "../hooks/useAuth";
import UsersService from "../services/users.service";
import { STATUS_STYLE } from "./swimmingPool/constants";
import { Pagination, TableSkeleton } from "./swimmingPool/shared";
import ActionMenu from "./swimmingPool/ActionMenu";
import SetupFormModal from "./swimmingPool/SetupFormModal";
import ReassignModal from "./swimmingPool/ReassignModal";
...
export default function SwimmingPoolInterface() {
  ...
  const setups = useAppSelector(selectSwimmingPoolSetups);
  const meta = useAppSelector(selectSwimmingPoolSetupsMeta);
  const loading = useAppSelector(selectSwimmingPoolLoading);
  const error = useAppSelector(selectSwimmingPoolError);
  const filters = useAppSelector(selectSwimmingPoolFilters);
  ...
  useEffect(() => {
    dispatch(fetchSwimmingPoolSetups());
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [dispatch, filters.page, filters.pool_location, filters.date_from, filters.date_to, filters.pool_type]);
  ...
  useEffect(() => {
    if (error) {
      toast.error(error);
      dispatch(clearSwimmingPoolError());
    }
  }, [error, dispatch]);
  ...
  function handleSearch(val) {
    ...
    searchTimer.current = setTimeout(
      () => dispatch(setSwimmingPoolFilter({ key: "pool_location", value: val })),
      400
    );
  }
  ...
  async function handleDelete() {
    ...
    setDeleting(true);
    const result = await dispatch(deleteSwimmingPoolSetup(deleteTarget.id));
    setDeleting(false);
    setDeleteTarget(null);
    if (deleteSwimmingPoolSetup.fulfilled.match(result))
  ...
  function handleExport() {
    const rows = [
      ["ID", "Pool Type", "Pool Size", "Pool Location", "Date", "Time", "Safety Officer", "Supervisor"],
      ...setups.map((s) => [
        s.id,
        s.pool_type ?? "",
        s.pool_size ?? "",
    ...
    const a = Object.assign(document.createElement("a"), {
      href: url,
      download: "swimming-pool-inspections.csv",
    });
  ...
  return (
    <div className="ui-page" style={{ color: "var(--text)" }}>
          ...
          <div>
            ...
            <p className="text-xs mt-0.5" style={{ color: "var(--text-muted)" }}>
              {meta
                ...
                : "Manage swimming pool inspection setups."}
            </p>
          </div>
        </div>
        <div className="flex items-center gap-2">
          <button
            onClick={() => dispatch(fetchSwimmingPoolSetups())}
            disabled={loading}
        ...
        <div className="flex flex-col sm:flex-row gap-3 items-stretch sm:items-center flex-wrap">
          ...
          >
            ...
            <input
              ...
              placeholder="Search by pool location…"
              style={{
            ...
            {searchInput && (
              <button
                onClick={() => {
                  setSearchInput("");
                  dispatch(setSwimmingPoolFilter({ key: "pool_location", value: "" }));
                }}
                style={{ color: "var(--text-muted)" }}
              >
          ...
          >
            ...
            <input
              ...
              onChange={(e) =>
                dispatch(setSwimmingPoolFilter({ key: "date_from", value: e.target.value }))
              }
              title="From date"
              style={{
                background: "transparent",
            ...
            <input
              ...
              onChange={(e) =>
                dispatch(setSwimmingPoolFilter({ key: "date_to", value: e.target.value }))
              }
              title="To date"
              style={{
                background: "transparent",
          ...
          {hasFilters && (
            <button
              onClick={() => {
                setSearchInput("");
                dispatch(clearSwimmingPoolFilters());
              }}
      ...
      <div className="ui-card">
                ...
                <tr>
                  ...
                  >
                    {hasFilters
                      ...
                      : "No swimming pool inspections yet."}
                  </td>
                </tr>
              </tbody>
            ) : (
                ...
                {setups.map((setup) => {
                  ...
                  return (
                    <tr key={setup.id} className="ui-row">
                      ...
                      >
                        {setup.pool_type
                          ? setup.pool_type.charAt(0).toUpperCase() + setup.pool_type.slice(1)
                          : "—"}
                      </td>
                      <td
                        className="ui-td text-sm"
                        style={{ color: "var(--text-muted)" }}
                      >
                        {setup.pool_size ?? "—"}
                      </td>
        ...
        <Pagination
          meta={meta}
          onPage={(p) => dispatch(setSwimmingPoolPage(p))}
        />
      ...
      <SetupFormModal
        ...
        onClose={() => {
          ...
          dispatch(clearSwimmingPoolActionError());
        }}
