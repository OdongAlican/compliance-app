/* ── Swimming Pool Inspection — StartInspectionModal ─────────────────── */
...
export default function StartInspectionModal({ isOpen, onClose, setup }) {
  ...
  /* Load checklist templates using setup's inspection_id (if linked) */
  useEffect(() => {
  ...
  async function handleSubmit() {
    ...
    try {
      ...
      await SwimmingPoolPerformService.create(setup.id, {
        perform: {
          date: form.date,
          time: form.time,
  ...
  return (
    <ModalShell
      isOpen={isOpen}
      onClose={onClose}
      title={"Start Inspection — " + (setup?.pool_location ?? "")}
      width="max-w-3xl"
    >
