/* ── Swimming Pool Inspection — ReassignModal ────────────────────────── */
import { useState, useEffect } from "react";
import { UserIcon } from "@heroicons/react/24/outline";
import { useAppDispatch, useAppSelector } from "../../store/hooks";
import {
  reassignSwimmingPoolSupervisor,
  reassignSwimmingPoolSafetyOfficer,
...
export default function ReassignModal({ isOpen, onClose, mode, setupId }) {
  ...
  async function handleSave() {
    ...
    const action = isSupervisor
      ? dispatch(reassignSwimmingPoolSupervisor({ id: setupId, supervisorId: Number(selectedUser.id) }))
      : dispatch(reassignSwimmingPoolSafetyOfficer({ id: setupId, safetyOfficerId: Number(selectedUser.id) }));
    const result = await action;
    if (
      reassignSwimmingPoolSupervisor.fulfilled.match(result) ||
      reassignSwimmingPoolSafetyOfficer.fulfilled.match(result)
    ) {
