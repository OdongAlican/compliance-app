/* ── Swimming Pool Inspection — DetailDrawer ─────────────────────────── */
...
import { SwimmingPoolPerformService } from "../../services/swimmingPool.service";
...
export default function DetailDrawer({ isOpen, onClose, setup }) {
  ...
  const load = useCallback(async () => {
    ...
    setError("");
    try {
      const res = await SwimmingPoolPerformService.list(setup.id, { per_page: 20 });
  ...
  async function handleSignOff(pid) {
    setSigning(pid);
    try {
      await SwimmingPoolPerformService.signOff(pid);
  ...
  const poolTypeLabel = setup.pool_type
      ...
      >
        ...
        >
          <div className="flex items-start gap-3">
            ...
            >
              <BuildingLibraryIcon className="w-5 h-5" style={{ color: "var(--accent)" }} />
            </div>
            <div>
              ...
              <p className="text-xs mt-0.5" style={{ color: "var(--text-muted)" }}>
                Swimming Pool Inspection · #{setup.id}
              </p>
              <span
        ...
        <div className="px-6 py-5" style={{ borderBottom: "1px solid var(--border)" }}>
          ...
          <div className="grid grid-cols-2 gap-3 mb-4">
            <ReviewRow label="Pool Type" value={poolTypeLabel} />
            <ReviewRow label="Pool Size" value={setup.pool_size} />
            <div className="col-span-2">
              <ReviewRow label="Pool Location" value={setup.pool_location} />
            </div>
