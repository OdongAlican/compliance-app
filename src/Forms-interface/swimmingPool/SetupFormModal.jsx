/* ── Swimming Pool Inspection — SetupFormModal ───────────────────────── */
...
import {
  ...
  selectSwimmingPoolActionLoading,
  selectSwimmingPoolActionError,
} from "../../store/slices/swimmingPoolSlice";
import toast from "react-hot-toast";
import { Field, Spinner, ModalShell, StepIndicator, ReviewRow } from "./shared";
import UserAutocomplete from "./UserAutocomplete";
import { POOL_TYPE_OPTIONS } from "./constants";

export default function SetupFormModal({ isOpen, onClose, setup }) {
  const dispatch = useAppDispatch();
  const actionLoading = useAppSelector(selectSwimmingPoolActionLoading);
  const actionError = useAppSelector(selectSwimmingPoolActionError);
    ...
    if (isOpen) {
      ...
      if (setup) {
        setForm({
          ...
          pool_size: setup.pool_size ?? "",
          pool_location: setup.pool_location ?? "",
          date: setup.date ?? "",
          time: setup.time ?? "",
          note: setup.note ?? "",
        });
        ...
        setForm({ pool_type: "", pool_size: "", pool_location: "", date: "", time: "", note: "" });
  ...
  function validateStep(s) {
    const e = {};
    if (s === 1) {
      if (!form.pool_type) e.pool_type = "Pool type is required.";
      if (!form.pool_size.trim()) e.pool_size = "Pool size is required.";
      if (!form.pool_location.trim()) e.pool_location = "Pool location is required.";
  ...
  async function handleSubmit() {
    const payload = {
      ...
      pool_size: form.pool_size.trim(),
      pool_location: form.pool_location.trim(),
      date: form.date,
      time: form.time,
    ...
    const action = isEdit
      ? dispatch(updateSwimmingPoolSetup({ id: setup.id, data: payload }))
      : dispatch(createSwimmingPoolSetup(payload));
    const result = await action;
    if (
      createSwimmingPoolSetup.fulfilled.match(result) ||
      updateSwimmingPoolSetup.fulfilled.match(result)
    ) {
    ...
    if (step === 1) {
      return (
        <div className="p-6 grid grid-cols-2 gap-4">
          <Field label="Pool Type" required error={errors.pool_type}>
            <select
          ...
          <Field label="Pool Size" required error={errors.pool_size}>
            <input
              value={form.pool_size}
              onChange={(e) => set("pool_size", e.target.value)}
              placeholder="e.g. 25m × 10m"
              className="ui-input text-sm"
            />
          </Field>
          <div className="col-span-2">
            <Field label="Pool Location" required error={errors.pool_location}>
              <input
                value={form.pool_location}
                onChange={(e) => set("pool_location", e.target.value)}
      ...
      return (
        <div className="p-6 flex flex-col gap-4">
          ...
          <div className="grid grid-cols-2 gap-3">
            <ReviewRow label="Pool Type" value={form.pool_type ? form.pool_type.charAt(0).toUpperCase() + form.pool_type.slice(1) : ""} />
            <ReviewRow label="Pool Size" value={form.pool_size} />
            <div className="col-span-2">
              <ReviewRow label="Pool Location" value={form.pool_location} />
            </div>
  ...
  return (
    <ModalShell
      isOpen={isOpen}
      onClose={onClose}
      title={isEdit ? "Edit Swimming Pool Inspection" : "New Swimming Pool Inspection"}
      width="max-w-lg"
    >
