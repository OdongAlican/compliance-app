...
function AuditorPicker({ selected, onToggle, label = "Assign Auditors" }) {
  ...
  function pick(u) {
    ...
    setQuery("");
    // Re-fetch so the toggled user's checked state refreshes
    clearTimeout(timer.current);
                    ...
                    return (
                      <li key={u.id}>
                        <button
                          ...
                          style={{ background: checked ? `${ACCENT}10` : "transparent" }}
        ...
        <div className="flex flex-wrap gap-1.5">
          {selected.map((u) => (
            <span
              ...
              style={{ background: ACCENT_LIGHT, color: ACCENT, border: `1px solid ${ACCENT}40` }}
            >
              {displayName(u)}
