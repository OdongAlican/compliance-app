...
function TcIssueCard({ issue: initialIssue, auditId, performedId }) {
  ...
  async function saveCa() {
    ...
    setSaving(false);
    if (updateTcCorrectiveAction.fulfilled.match(r)) {
  ...
  async function savePriority() {
    ...
    setSaving(false);
    if (updateTcPriorityDueDate.fulfilled.match(r)) {
  ...
  async function saveContractor() {
    ...
    setSaving(true);
    const r = await dispatch(assignTcContractor({ auditId, performedId, issueId: issue.id, contractorId: contractor.id }));
    setSaving(false);
    if (assignTcContractor.fulfilled.match(r)) {
  ...
  async function saveExecute() {
    ...
    setSaving(true);
    const r = await dispatch(executeTcIssue({
    ...
    setSaving(false);
    if (executeTcIssue.fulfilled.match(r)) {
        ...
        <div className="px-4 pb-4 flex flex-col gap-3" style={{ borderTop: "1px solid var(--border)" }}>
          ...
          {!isClosed && (
            <div className="flex flex-wrap gap-1.5 pt-1">
              {TC_ISSUE_ACTIONS.map(({ key, label, Icon }) => {
              ...
              <div className="p-4">
                ...
                <div className="flex justify-end mt-3">
                  <button onClick={saveCa} disabled={saving || !caText.trim()}
                    ...
                    style={{ background: ACCENT, opacity: saving || !caText.trim() ? 0.5 : 1, boxShadow: `0 2px 10px rgba(22,163,74,0.35)` }}>
              ...
              <div className="p-4">
                ...
                <div className="flex justify-end mt-3">
                  <button onClick={savePriority} disabled={saving}
                    className="flex items-center gap-1.5 text-xs font-semibold px-4 py-2 rounded-full text-white"
                    style={{ background: ACCENT, opacity: saving ? 0.5 : 1, boxShadow: `0 2px 10px rgba(22,163,74,0.35)` }}>
                    {saving ? <Spinner size={3} /> : <CheckCircleIcon className="h-3.5 w-3.5" />} Save
                  </button>
                </div>
              </div>
              ...
              <div className="p-4">
                ...
                <div className="flex justify-end mt-3">
                  <button onClick={saveContractor} disabled={saving || !contractor}
                    className="flex items-center gap-1.5 text-xs font-semibold px-4 py-2 rounded-full text-white"
                    style={{ background: ACCENT, opacity: saving || !contractor ? 0.5 : 1, boxShadow: `0 2px 10px rgba(22,163,74,0.35)` }}>
...
function PerformedDetailModal({ isOpen, onClose, performed, auditId }) {
  ...
  const issuesMap = useAppSelector(selectTcIssuesByPerformed);
  ...
  const issues = storedIssues.length ? storedIssues : (performed?.performed_training_and_competency_issues ?? []);
  ...
  useEffect(() => {
    ...
    if (activeTab === "issues" && !storedIssues.length) {
      setLoading(true);
      dispatch(fetchTcIssues({ auditId, performedId: performed.id }))
          ...
          {activeTab === "issues" && (
            <div className="p-5">
              {loading ? (
                ...
                <div className="flex flex-col gap-3">
                  {issues.map((iss) =>
                    (can("performed_training_and_competency_issues.corrective_action") || can("performed_training_and_competency_issues.priority_and_due_date") || can("performed_training_and_competency_issues.assign_contractor") || can("performed_training_and_competency_issues.execute")) ? (
                      ...
                      <div key={iss.id}
                        ...
                        <div className="flex-1 min-w-0">
                          <p className="text-sm font-semibold" style={{ color: "var(--text)" }}>{iss.name}</p>
                          {iss.corrective_action && (
                            <p className="text-xs mt-0.5 leading-relaxed" style={{ color: "var(--text-muted)" }}>{iss.corrective_action}</p>
                          )}
                        </div>
